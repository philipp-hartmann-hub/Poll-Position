...
# Modellannahme für die 18-Uhr-Prognose, nicht institutionell verifiziert.
# Orientierung: infratest dimap nennt Ø ~0,5 Pp Abweichung vom Endstand; wir setzen
# bewusst höher (1,5–2 Pp), ohne Instituts-house_variance-Aufschlag.
WAHLABEND_SD_PP = 1.75
WAHLABEND_SAMPLE_SIZE = 1000


def wahlabend_house_variance(mean_share: float) -> float:
    """
    house_variance so, dass ``total_sd_pp(mean, WAHLABEND_SAMPLE_SIZE, hv)``
    ≈ ``WAHLABEND_SD_PP`` (keine zusätzliche Institutsstreuung).
    """
    se = standard_error_pp(mean_share, WAHLABEND_SAMPLE_SIZE)
    return max(0.0, WAHLABEND_SD_PP**2 - se**2)
...
def party_uncertainties_election_night(
    ...
    """
    Wahlabend: Ziel-SD ``WAHLABEND_SD_PP`` über ``party_uncertainties_from_means``.

    house_variance wird je Partei so gewählt, dass SE + HV die feste 18-Uhr-SD
    ergeben — ohne zusätzlichen Instituts-Aufschlag (Modellannahme).
    """
    hv = {pid: wahlabend_house_variance(float(share)) for pid, share in means.items()}
    return party_uncertainties_from_means(
        means,
        sample_size=WAHLABEND_SAMPLE_SIZE,
        house_variance=hv,
    )
